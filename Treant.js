...
(function() {
  ...
  Tree.prototype = {
    ...
    positionNodes: function() {
      ...
      var treeWidth = treeSize.x.max - treeSize.x.min;
      var treeHeight = treeSize.y.max - treeSize.y.min;
      ...
      // TODO # add new config to force the tree is center in edge when the tree is smaller than the draw area.
      var deltaX = containerCenter.x - treeCenter.x;
      var deltaY = containerCenter.y - treeCenter.y;

      // all nodes must have positive X or Y coordinates, handle this with offsets
      var negOffsetX = ((treeSize.x.min + deltaX) <= 0) ? Math.abs(treeSize.x.min) : 0;
      var negOffsetY = ((treeSize.y.min + deltaY) <= 0) ? Math.abs(treeSize.y.min) : 0;
      var i;
      var len;
      var node;

      console.log('negOffsetX: ' + negOffsetX);
      console.log('negOffsetY: ' + negOffsetY);
      ...
      for (i = 0, len = this.nodeDB.db.length; i < len; i++) {
        ...
        if(this.CONFIG.rootSticky) {
          console.log(node);

          if(this.CONFIG.rootOrientation === 'NORTH') {
            node.X += negOffsetX + ((treeWidth < this.drawArea.clientWidth) ? deltaX : this.CONFIG.padding);
            node.Y += negOffsetY + this.CONFIG.padding;
          } else if(this.CONFIG.rootOrientation === 'WEST') {
            node.X += negOffsetX + this.CONFIG.padding;
  ...
  NodeDB.prototype = {
    ...
    getMinMaxCoord: function(dim, parent, MinMax) {
      ...
      if(!parent) {
        var check = this.get(0);
        console.log('check.min.' + dim + ' = ' + check[dim]);
      }
  ...
  Tree.CONFIG = {
    ...
    rootSticky: false, // Let the root stick on edage. Only works when the root orientation is NORTH or WEST.
